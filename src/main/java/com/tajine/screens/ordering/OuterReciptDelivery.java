...
public class OuterReciptDelivery  {
	...
	public void printRecipt(String printerName, Order order, String cashier, String counter) throws JRException {
		...
		try (InputStream in = getClass().getResourceAsStream("/outer_delivery.jrxml")) {
			...
			parameters.put("PHONE", order.getCustomer().getPhone());
			parameters.put("CUSTOMER_NAME", order.getCustomer().getName());
			parameters.put("DELIVERY_FEES", String.valueOf(order.getDeliveryFees()));
			parameters.put("TOTAL_TOTAL", String.valueOf(order.getDeliveryFees() + order.getTotalPrice()));
